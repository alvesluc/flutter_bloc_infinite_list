...
void main() {
  ...
  group('PostsList', () {
    testWidgets(
      'renders CircularProgressIndicator when post status is initial',
      (tester) async {
    ...
    testWidgets(
      'renders no post text when post status is success but with 0 posts',
      (tester) async {
        when(() => postBloc.state).thenReturn(const PostState(
    ...
    testWidgets(
      'does not renders a BottomLoader when post max is reached',
      (tester) async {
        when(() => postBloc.state).thenReturn(PostState(
          status: PostStatus.success,
          posts: mockPosts,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpPostsList(postBloc);
        await tester.drag(find.byType(PostsList), Offset(0, -500));
